...
class XMiniGridWrapper(Wrapper):
    ...
    def reset(self, key: chex.PRNGKey) -> Tuple[XMiniGridEnvState, TimeStep]:
        ...
        obs = Observation(
            minigrid_state_timestep.observation, self._legal_action_mask, jnp.array(0)
        )
        timestep = TimeStep(
            observation=obs,
    ...
    def step(self, state: XMiniGridEnvState, action: chex.Array) -> Tuple[State, TimeStep]:
        ...
        timestep = TimeStep(
            observation=Observation(
                minigrid_state_timestep.observation,
    ...
    def observation_spec(self) -> Spec:
        return specs.Spec(
            ...
            "ObservationSpec",
            agent_view=Array(
                shape=self._env.observation_shape(self._env_params), dtype=jnp.float32
            ),
