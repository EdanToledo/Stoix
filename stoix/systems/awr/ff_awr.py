...
def get_learner_fn(
    ...
    def _update_step(learner_state: AWRLearnerState, _: Any) -> Tuple[AWRLearnerState, Tuple]:
        ...
        def _update_critic_step(update_state: Tuple, _: Any) -> Tuple:
            ...
            d_t = 1 - sequence.done.astype(jnp.float32)[:, :-1]
            last_v = v_t[:, -1]
            v_t = v_t[:, :-1]
            swap_axes = lambda x: jnp.swapaxes(x, 0, 1)
            v_t, r_t, d_t = jax.tree_map(swap_axes, (v_t, r_t, d_t))
            _, target_vals = calculate_gae(v_t, r_t, d_t, last_v, config.system.gae_lambda)
            target_vals = jnp.swapaxes(target_vals, 0, 1)

            # CALCULATE CRITIC LOSS
            critic_grad_fn = jax.grad(_critic_loss_fn, has_aux=True)
            critic_grads, q_loss_info = critic_grad_fn(
            ...
            critic_grads, critic_loss_info = jax.lax.pmean(
                (critic_grads, q_loss_info), axis_name="batch"
            )
            critic_grads, critic_loss_info = jax.lax.pmean(
                (critic_grads, q_loss_info), axis_name="device"
            )
            ...
            return (new_params, new_opt_state, buffer_state, key), critic_loss_info
        ...
        def _update_actor_step(update_state: Tuple, _: Any) -> Tuple:
            ...
            swap_axes = lambda x: jnp.swapaxes(x, 0, 1)
            v_t, r_t, d_t = jax.tree_map(swap_axes, (v_t, r_t, d_t))
            advantages, _ = calculate_gae(v_t, r_t, d_t, last_v, config.system.gae_lambda)
