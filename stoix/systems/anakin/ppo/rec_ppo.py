...
from stoix.systems.anakin.ppo.ppo_types import ActorCriticHiddenStates, RNNPPOTransition
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _actor_loss_fn(
                    ...
                    policy_hidden_state = jax.tree_util.tree_map(lambda x: x[0], traj_batch.hstates.policy_hidden_state)
                    _, actor_policy = actor_apply_fn(actor_params, policy_hidden_state, obs_and_done)
                ...
                def _critic_loss_fn(
                    ...
                    reset_hidden_state = jnp.logical_or(traj_batch.done, traj_batch.truncated)
                    obs_and_done = (traj_batch.obs, reset_hidden_state)
                    critic_hidden_state = jax.tree_util.tree_map(lambda x: x[0], traj_batch.hstates.critic_hidden_state)
...
def run_experiment(_config: DictConfig) -> float:
    ...
    # Setup learner.
    learn, actor_network, actor_rnn, learner_state = learner_setup(env, (key, actor_net_key, critic_net_key), config)
