...
from stoix.systems.anakin.mpo.continuous_loss import clip_dual_params, mpo_loss
    ...
    def _update_step(learner_state: MPOLearnerState, _: Any) -> Tuple[MPOLearnerState, Tuple]:
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _actor_loss_fn(
                ...
                target_sampled_actions = target_actor_policy.sample(seed=key, sample_shape=config.system.num_samples)
                target_sampled_q_values = jax.vmap(q_apply_fn, in_axes=(None, None, 0))(
                    target_q_params,
                    reshaped_obs,
                    target_sampled_actions.clip(config.system.action_minimum, config.system.action_maximum),
                )
            ...
            def _q_loss_fn(
                ...
                online_actor_policy = actor_apply_fn(online_actor_params, sequences.obs)  # [B, T, ...]
                target_actor_policy = actor_apply_fn(target_actor_params, sequences.obs)  # [B, T, ...]
                online_q_t = q_apply_fn(
                    online_q_params,
                    sequences.obs,
                    sequence.action.clip(config.system.action_minimum, config.system.action_maximum),
                )  # [B, T]
                ...
                q_values = jax.vmap(q_apply_fn, in_axes=(None, None, 0))(
                    target_q_params,
                    sequences.obs,
                    a_evaluation.clip(config.system.action_minimum, config.system.action_maximum),
                )
                ...
                if config.system.use_retrace:
                    ...
                    target_q_t = q_apply_fn(
                        target_q_params,
                        sequences.obs,
                        sequences.action.clip(config.system.action_minimum, config.system.action_maximum),
                    )  # [B, T]
...
@hydra.main(config_path="../../../configs", config_name="default_ff_mpo_continuous.yaml", version_base="1.2")
