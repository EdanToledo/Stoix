...
def make_downsampling_layer(
  ...
  if strategy is DownsamplingStrategy.AVG_POOL:
    ...
    return nn.Sequential([
        ...
        lambda x : nn.max_pool(x, window_shape=(3, 3, 1), strides=(2, 2, 1), padding='SAME')
    ])
  else:
...
class VisualResNetTorso(nn.Module):
    ...
    def __call__(self, observation: chex.Array) -> chex.Array:
        ...
        output = observation
        channels_blocks_strategies = zip(self.channels_per_group,
                                        self.blocks_per_group,
                                        self.downsampling_strategies)

        for _, (num_channels, num_blocks,
